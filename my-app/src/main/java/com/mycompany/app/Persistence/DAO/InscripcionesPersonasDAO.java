...
public class InscripcionesPersonasDAO {
  ...
  public InscripcionesPersonasDAO(Connection connection) {
    this.connection = connection;
  }

  public void insertar(InscripcionDTO i) {
    String sql = "INSERT INTO inscripciones_personas (curso_id, persona_id, año, semestre) VALUES (?, ?, ?, ?)";
  ...
  public void eliminar(Double id) {
    String sql = "DELETE FROM inscripciones_personas WHERE id=?";
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
      ps.setDouble(1, id);
      ps.executeUpdate();
    } catch (SQLException e) {
